...
import type { Rune, RuneTree } from '../dragon';
...
export class CurrentGamePerks {
  ...
  constructor(runeTrees: Collection<string, RuneTree>, data: CurrentGamePerksData) {
    ...
    const stat = data.perkIds.filter((p) => Object.keys(rawStatPerks).includes(p.toString()));
    this.stats = stat.map((p) => rawStatPerks[p]);
